...
func vectorize(body []byte, out *[Dims]int16) bool {
	p := 0
	...
	inst, next, ok := parseFloat(body, p)
	if !ok {
		return false
	}
	out[1] = quantClamp01(inst / maxInstallments)
	p = next
	...
	if mcc < 0 || mcc >= 10000 {
		out[12] = quantClamp01(defaultMccRisk)
	} else {
